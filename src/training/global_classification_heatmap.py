...
def main(args):
    ...
    if (args.mode == 'train' or args.mode == 'sc'):
        image_root = os.path.join(processed_db_dir ,args.image_root)
        heatmap_root = os.path.join(processed_db_dir ,args.heatmap_root)
        ...
        args.dataloader_train = get_dataloader_train(args, image_root , heatmap_root, train_file)
        args.dataloader_test  = get_dataloader_test (args, image_root , heatmap_root, val_file)
        args.dataloader_val   = get_dataloader_test (args, image_root , heatmap_root, val_file)
...
def get_dataloader_train(args , image_root, heatmap_root , image_list):
    # kwargs = {'num_workers': 4, 'pin_memory': True} if args.cuda else {}
    kwargs = {'num_workers': args.num_workers  , 'pin_memory': args.pin_memory}


    dataset = ds.ClassificationDatasetMulti(image_root, heatmap_root,
                                image_list,
                                transform=transforms.Compose([
                                    # multi_transforms.ResizeMulti(256),
                                    # multi_transforms.RandomRotationMulti(5),
                                    # transforms.RandomCrop(224),
                                    multi_transforms.RandomResizedCropMulti(224, scale=(0.74, 0.90),
                                                                 ratio=(.9, 1.0)),
...
def get_dataloader_test(args, image_root, heatmap_root , image_list):
...
class ClassificationTrainer(object):
    ...
    def get_model(self):
        ...
        if self.args.architecture in ['densenet121', 'densenet169', 'densenet201', 'densenet161']:
            ...
            nn.init.kaiming_normal(model.features.conv0.weight.data)
...
class ClassificationArguments():
    ...
    def initialize(self):
        ...
        imp_arg.add_argument("-hr", '--heatmap_root', default='def',
                             metavar='N',
                             help='root directory of the image')
