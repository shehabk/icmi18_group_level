    ...
    if (args.architecture in [ 'resnet_i48_18' ]):
        ...
        dataset = ds.ClassificationDataset(root,
                                    ...
                                    transform=transforms.Compose([
                                        ...
                                        transforms.RandomResizedCrop(224, scale=(0.74, 0.85),
                                                                     ratio=(1.0, 1.0)),
                                        transforms.RandomHorizontalFlip(),
                                        transforms.ToTensor(),
                                        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
...
class ClassificationTrainer(object):
    ...
    def run_train(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs + 1):
            ...
            # testloss, testacc = self.evaluate(self.dataloader_test, self.model, self.criterion)
            # test_acc = testacc.avg
            # is_best_test = test_acc >= self.best_acc_test
            # self.best_acc_test = max(test_acc, self.best_acc_test)
            ...
            }, is_best, is_best_test=is_best_test)
            ...
            #     testloss.avg, int(testloss.sum), test_acc.count,
            #     100. * test_acc.avg))
    ...
    def run_write_score( self ):
        ...
        self.write_score(self.dataloader_val, self.model )
        if self.args.copy_back:
            self.pool.apply_async(methods_util.copy_to_shehabk ,[self.args.exp_dir])
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if batch_idx % self.args.log_interval == 0:
                print('Train {} Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    self.args.log_prefix, epoch, batch_idx * len(data), len(train_loader.sampler),  # do train_loader.sampler
    ...
    def save_checkpoint(self, state, is_best , is_best_test = False,filename='checkpoint.pth.tar',
                        best_filename='model_best.pth.tar' , best_filename_test='model_best_test.pth.tar'):
        ...
        best_filename_test = os.path.join(models_dir, best_filename_test)
        # -------------------------------------------------
        torch.save(state, filename)
        if is_best:
            shutil.copyfile(filename, best_filename)
        if is_best_test:
            shutil.copyfile(filename, best_filename_test)
    ...
    def get_model(self):
        ...
        if self.args.architecture in ['resnet_i48_18']:
            model  = torchmodels.resnet18()
            model  = Renset_i48( model , num_classes= self.args.num_classes)
            processed_db_dir = project_dir
            model_path = os.path.join(processed_db_dir,
                                      'models/pretrained/raf_fer2013/resnet_i48_18/'
                                      '%s/'
                                      'models/model_best.pth.tar' % (args.experiment_padding))
            model.load_state_dict(torch.load(model_path)['state_dict'])

        if self.args.architecture in ['resnet_i24_34']:
            model  = torchmodels.resnet34()
            model  = Renset_i24( model , num_classes= self.args.num_classes , input_size=24)
            processed_db_dir = project_dir
            model_path = os.path.join(processed_db_dir,
                                      'models/pretrained/raf_fer2013/resnet_i24_34/'
                                      '%s/'
                                      'models/model_best.pth.tar' % (args.experiment_padding))
            model.load_state_dict(torch.load(model_path)['state_dict'])
    ...
    def get_optimizer(self):
        ...
        # filter parameters by name for zibonet
        # add much higher learning rate in fc layers
        param_list = list()
        if self.args.architecture in ['zibonet']:
            fc_params = list()
            conv_params = list()
            ...
            param_list.append({'params': fc_params, 'lr': self.args.lr})
            param_list.append({'params': conv_params, 'lr': self.args.lr / 1.0})

        elif 'resnet_i48_18' in  self.args.architecture:
            # print ('Here')
            begin = list()
            layer1 = list()
            layer2 = list()
            ...
            param_list.append({'params': begin,  'lr': self.args.lr * 0.1})
            param_list.append({'params': layer1, 'lr': self.args.lr * 0.1})
            param_list.append({'params': layer2, 'lr': self.args.lr * 0.1})
            param_list.append({'params': layer3, 'lr': self.args.lr * 1.0})
            param_list.append({'params': layer4, 'lr': self.args.lr * 1.0})
            param_list.append({'params': fc, 'lr': self.args.lr * 1.0})

        else:
            param_list.extend(self.model.parameters())
