...
def main():
    ...
    # image_list = 'emotiw_2018/partition/global/test_orig.txt'
...
#
    ...
    bwg   = args.global_weight
    bwgb  = args.global_blurred_weight
    bwgh  = args.global_heatmap_weight
    bwfa  = args.faces_weight

    # print (bwg , bwgb , bwgh , bwfa)
    ...
    sub_dir_full = os.path.join(project_dir, sub_dir, sub_name)
    prepare_submission(sub_dir_full, weighted_output, image_paths, labels)
...
def prepare_submission( sub_dir_full , outputs , image_paths , labels ):
    assert outputs.shape[0] == len(image_paths)
    if not os.path.exists(sub_dir_full):
    ...
    for i in range( len(image_paths)):
        ...
        file_name = os.path.join( sub_dir_full , image_name + '.txt')
...
def parse_arguments():
    ...
    args.add_argument("--sub_dir", metavar='N', default='data/submission',
                         help='alligned')
    args.add_argument("--sub_name", metavar='N', default='val',
                         help='alligned')
    ...
    args.add_argument("--global_weight", default=0.10, type=float,
                        metavar='W', help='epoch-length (default: 2000)')
    args.add_argument("--global_blurred_weight", default=0.07, type=float,
                        metavar='W', help='epoch-length (default: 2000)')
    args.add_argument("--global_heatmap_weight", default=0.09, type=float,
                        metavar='W', help='epoch-length (default: 2000)')
    args.add_argument("--faces_weight", default=0.74, type=float,
                        metavar='W', help='epoch-length (default: 2000)')
