...
def main():
    ...
    pretrained_model = 'models/global/%s/%s/default/models/%s'%(trained_on , arch , model_name)
    store_outputs    = 'data/predictions/global/%s/%s/%s' %(   trained_on,
                                                                                  test_data,
                                                                                  arch + arch_pad)
    ...
    for id in range( len(image_paths)):
        ...
        blank_label  = np.zeros((1,1) , dtype=np.int32  )
    ...
    output_global_fl = os.path.join(store_outputs_fullpath , 'output_global.txt' )
    ...
    print ( "Global Accuracy: " + str(get_accuracy(outputs , filtered_labels)))
